...
class FashionMNISTObjective:
    ...
    def __init__(self, settings: HPOSettings) -> None:
        ...
        self.train_loader = DataLoader(Subset(dataset, train_indices), batch_size=settings.train_batch_size, shuffle=True, generator=generator)
        self.validation_loader = DataLoader(Subset(dataset, validation_indices), batch_size=settings.validation_batch_size, shuffle=False)
...
def interpolate_run(history_evaluations: List[int], history_loss: List[float], common_x: np.ndarray) -> np.ndarray:
    interpolator = interpolate.interp1d(history_evaluations, history_loss, kind='previous', bounds_error=False, fill_value=(history_loss[0], history_loss[-1]))
    return interpolator(common_x)

def process_multi_runs(results_dict: Dict[str, List[Tuple[List[int], List[float]]]], budget: int) -> Dict[str, Dict[str, np.ndarray]]:
    ...
    for algorithm_name, runs in results_dict.items():
        ...
        processed[algorithm_name] = {'x': common_x, 'mean': np.mean(matrix, axis=0), 'min': np.min(matrix, axis=0), 'max': np.max(matrix, axis=0)}
    return processed
...
def run_robust_benchmark(settings: Optional[HPOSettings]=None) -> Tuple[Dict[str, Dict[str, np.ndarray]], Dict[str, List[float]], Dict[str, List[Tuple[List[int], List[float]]]]]:
    config = settings or HPOSettings()
    objective = FashionMNISTObjective(config)
    bounds = [(0.0, 10.0)] * config.dimension
    times: Dict[str, List[float]] = {'CDS': [], 'Random Search': [], 'Bayesian Optimization': []}
    raw_results: Dict[str, List[Tuple[List[int], List[float]]]] = {'CDS': [], 'Random Search': [], 'Bayesian Optimization': []}
    ...
    for run_index, seed in enumerate(config.seeds, start=1):
        ...
        def cds_wrapper(point: np.ndarray) -> float:
            ...
            return value
        cds = BoxCellularDirectSearch(bounds=bounds, step_size=config.step_size, objective_function=cds_wrapper, num_active_cells=config.num_active_cells, seed=seed)
        ...
        try:
            ...
            gp_minimize(bo_wrapper, space, n_calls=config.budget, n_initial_points=10, random_state=seed)
...
def run_quick_benchmark(settings: Optional[HPOSettings]=None) -> Tuple[Dict[str, Dict[str, np.ndarray]], Dict[str, List[float]], Dict[str, List[Tuple[List[int], List[float]]]]]:
    config = settings or HPOSettings(seeds=(42,), budget=20, epochs=1, train_size=1000, validation_size=250, train_batch_size=64, validation_batch_size=250)
    return run_robust_benchmark(config)

def main() -> None:
    run_robust_benchmark()
__all__ = ['FashionMNISTObjective', 'HPOSettings', 'SimpleCNN', 'generate_pgfplots_code', 'interpolate_run', 'main', 'plot_processed_results', 'process_multi_runs', 'run_quick_benchmark', 'run_robust_benchmark']
