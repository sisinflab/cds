...
def build_problem_suite(dim: int) -> List[ProblemSpec]:
    return [
        ...
        *[build_bbob_problem(func_id=fid, dim=dim) for fid in range(1, 25)],
        build_abs_problem(dim=dim),
        build_layeb_problem(dim=dim)
    ]

def run_cds(problem: ProblemSpec, radius: float, budget: int, seed: int, step_size: float, num_cells: int) -> Tuple[
    np.ndarray, float]:
...
def run_cma_es(problem: ProblemSpec, radius: float, budget: int, seed: int, sigma_scale: float) -> Tuple[
    np.ndarray, float]:
    try:
        import cma
    except ImportError:
...
def run_pso(problem: ProblemSpec, radius: float, budget: int, seed: int, swarmsize: int) -> Tuple[np.ndarray, float]:
    ...
    while tracker.evaluations < budget:
        ...
        pso(tracker, -radius * np.ones(problem.dimension), radius * np.ones(problem.dimension), swarmsize=swarmsize,
            maxiter=max_iterations, ieqcons=[sphere_con], debug=False)
        if tracker.evaluations <= prev + swarmsize: break
        current_seed += 1
    return tracker.history_array(), tracker.elapsed_time()


def run_differential_evolution(problem: ProblemSpec, radius: float, budget: int, seed: int, popsize: int = 15) -> Tuple[
    np.ndarray, float]:
    tracker = ObjectiveTracker(problem.objective)
    ...
    while tracker.evaluations < budget:
        ...
        try:
            differential_evolution(tracker, [(-radius, radius)] * problem.dimension, maxiter=max_iterations,
                                   popsize=popsize, seed=current_seed, polish=False, constraints=(nlc,))
        except Exception:
            pass
        if tracker.evaluations <= prev + popsize: break
        current_seed += 1
    return tracker.history_array(), tracker.elapsed_time()
    ...
    while tracker.evaluations < budget:
        ...
        try:
            minimize(
                ...
                method='Nelder-Mead',
                bounds=bounds,
                options={"maxfev": budget - tracker.evaluations, "disp": False},
                tol=1e-6
            )
        except Exception:
            pass
...
def run_pdfo_baseline(problem: ProblemSpec, radius: float, budget: int, seed: int) -> Tuple[np.ndarray, float]:
    ...
    while tracker.evaluations < budget:
        ...
        pdfo.pdfo(tracker, x0, bounds=np.array([[-radius, radius]] * problem.dimension), constraints=[nlc],
                  options={"maxfev": budget - tracker.evaluations})
        if tracker.evaluations <= prev + 1: break
        current_seed += 1
    return tracker.history_array(), tracker.elapsed_time()
...
def run_lshade_baseline(problem: ProblemSpec, radius: float, budget: int, seed: int, pop_factor: int) -> Tuple[
    np.ndarray, float]:
    try:
        from mealpy.evolutionary_based import SHADE; from mealpy import FloatVar
    except ImportError:
        return np.empty((0, 2)), 0.0
    tracker = ObjectiveTracker(problem.objective)
...
def run_nomad_baseline(problem: ProblemSpec, radius: float, budget: int, seed: int) -> Tuple[np.ndarray, float]:
    ...
    while tracker.evaluations < budget:
        ...
        params = ["BB_OUTPUT_TYPE OBJ", f"MAX_BB_EVAL {budget - tracker.evaluations}", "DISPLAY_DEGREE 0",
                  f"SEED {current_seed}", "DIRECTION_TYPE ORTHO 2n", "QUAD_MODEL_SEARCH NO"]
        try:
...
def run_random_search(problem: ProblemSpec, radius: float, budget: int, seed: int) -> Tuple[np.ndarray, float]:
    ...
    while tracker.evaluations < budget:
        ...
        tracker(u / np.linalg.norm(u) * (rng.uniform(0, 1) ** (1. / problem.dimension)) * radius)
    return tracker.history_array(), tracker.elapsed_time()
...
def run_grid_search_baseline(problem: ProblemSpec, radius: float, budget: int, seed: int, step_size: float) -> Tuple[
    ...
    while tracker.evaluations < budget:
        u = rng.standard_normal(problem.dimension)
        p = u / np.linalg.norm(u) * (rng.uniform(0, 1) ** (1. / problem.dimension)) * radius
...
def build_optimizer_configs(settings: BenchmarkSettings) -> List[OptimizerConfig]:
    ...
    if settings.include_bads: configs.append(OptimizerConfig("BADS", run_bads_baseline, {}))
    if settings.include_nomad: configs.append(OptimizerConfig("NOMAD", run_nomad_baseline, {}))
    if settings.include_pdfo: configs.append(OptimizerConfig("Powell (PDFO)", run_pdfo_baseline, {}))
    if settings.include_neldermead: configs.append(OptimizerConfig("Nelder-Mead", run_neldermead_baseline, {}))
    if settings.include_de: configs.append(OptimizerConfig("DE (pop=15d)", run_differential_evolution, {"popsize": 15}))
    if settings.include_grid_search: configs.append(
        OptimizerConfig("Pure Grid Search (h=0.25)", run_grid_search_baseline, {"step_size": 0.25}))
    if settings.include_random: configs.append(OptimizerConfig("Random Search", run_random_search, {}))
    return configs
                ...
                for opt in optimizer_configs:
                    ...
                    for seed in config.seeds:
                        ...
                        record = {
                            ...
                            "Optimizer": opt.name,
                            "Seed": seed,
                            "Final Loss": float(hist[-1, 1]) if hist.size > 0 else float(np.inf),
                            "Total Evals": int(hist[-1, 0]) if hist.size > 0 else int(config.budget_evaluations),
                            "Time (s)": float(t),
                        }
                        ...
                        if csv_writer is not None:
                            ...
                            csv_handle.flush()
                        print(
                            f"     done | loss={record['Final Loss']:.6e} | "
                            f"evals={record['Total Evals']} | time={record['Time (s)']:.2f}s"
                        )
...
def create_summary_table(df: pd.DataFrame) -> pd.DataFrame:
    ...
    summary = df_best_params.groupby(['Problem', 'Dimension', 'Optimizer_Base']).agg(mu=('Final Loss', 'mean'),
                                                                                     sigma=('Final Loss', 'std'),
                                                                                     evals=('Total Evals',
                                                                                            'mean')).reset_index()
