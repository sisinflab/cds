...
DEFAULT_COMPLETE_CSV = DEFAULT_RESULTS_DIR / "cds_benchmarking_complete_results.csv"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Optional complete benchmark CSV to include in the combined wall-clock time table. "
            f"Typical value: {DEFAULT_COMPLETE_CSV}"
        ),
    )
...
def clean_complete_problem_name(problem_name: str) -> str:
    return re.sub(r"\s*\(?[\d]+D\)?\s*", "", str(problem_name)).strip()


def get_complete_problem_family(problem_name: str) -> str:
...
def get_complete_true_optimum(problem_name: str, dimension: int) -> float:
...
def read_complete_results(path: Path) -> tuple[pd.DataFrame, int, str | None]:
    df = pd.read_csv(path, sep=";")
    ...
    df["Source"] = df["Problem"].apply(clean_complete_problem_name)
    df["Family"] = df["Source"].apply(get_complete_problem_family)
    df["Instance"] = 0
    df["Rotated"] = 0
    df["Shift Norm"] = 0.0
    df["Transform"] = "complete"
    df["Dataset"] = "complete_non_directgolib"
    df["Optimizer"] = df["Optimizer"].astype(str)
    df["Optimizer_Base"] = df["Optimizer"].apply(get_base_optimizer)
    df["Optimizer_Config_Key"] = df["Optimizer"].apply(get_optimizer_config_key)

    df["True Opt"] = df.apply(
        lambda row: get_complete_true_optimum(row["Source"], row["Dimension"]),
        axis=1,
    )
    min_found = df.groupby(["Source", "Dimension", "Seed"])["Final Loss"].transform("min")
    ...
    if ioh is None and df["Source"].astype(str).str.contains("BBOB", case=False, na=False).any():
        warning = (
            "ioh is not available: BBOB gaps in the complete CSV use the per-seed empirical "
...
def build_combined_time_dataset(direct_df: pd.DataFrame, complete_csv: Path) -> tuple[pd.DataFrame, int, str | None]:
    complete_df, excluded_count, warning = read_complete_results(complete_csv)
    ...
    combined = pd.concat(
        [complete_df[common_columns], direct_combined[common_columns]],
        ignore_index=True,
    )
    return combined, excluded_count, warning


def print_combined_time_analysis(direct_df: pd.DataFrame, complete_csv: Path, dimension: int) -> None:
    combined, excluded_count, warning = build_combined_time_dataset(direct_df, complete_csv)
    ...
    print(f"Excluded complete DIRECTGOLib/ABS/Layeb rows: {excluded_count:,}")
    print(
...
def main() -> None:
    ...
    if args.complete_csv is not None:
        print_combined_time_analysis(df, args.complete_csv, dimension)
